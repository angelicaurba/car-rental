...
import { FaRegUser, FaCarSide } from 'react-icons/fa';
...
class AppNavbar extends React.Component {
    ...
    render() {
        return <Navbar className={"row"} bg="primary" expand="true">
            ...
            {this.state.path.endsWith("newrental") ?
            ...
            {(this.state.path.endsWith("rentals")) ?
                <Link to={"/user/newrental"} className={"col-2"}>
                    <Button className={"row"} variant="outline-light">
                        <FaCarSide className={"col-4"}/>
                        <span className={"col-6"}>Rent a car</span>
                    </Button>
                </Link>
                : <></>
            }
